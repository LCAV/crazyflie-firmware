...
#define AUDIO_TASK_FREQUENCY 100 // frequency at each packets are sent
...
void send_corr_packet(uint8_t channel){
	...
	if (packet_count == NPACKETS){ 	// send last packet and reset counter
...
void audio_deckTask(void* arg){
  ...
  while (1) {
    ...
    if (!corr_matrix_sending){
    	i2cdevRead(I2C1_DEV, AUDIO_DECK_ADDRESS, ARRAY_SIZE, corr_array); // get array from deck
